...
let MoodService = MoodService_1 = class MoodService {
    ...
    constructor(prisma, config, kakao, googlePlaces, placeService, creditService) {
        this.prisma = prisma;
        this.config = config;
        this.kakao = kakao;
    ...
    async search(query, userId, lat, lng, limit = 20, radius) {
        ...
        this.saveMoodSearchLog(query, analysis.summary, userId).catch((err) => this.logger.error('검색 로그 저장 실패', err));
        return {
    ...
    async searchKakaoPlaces(keywords, lat, lng, limit, radius) {
        const resultSets = await Promise.all(keywords.map((kw) => this.kakao.searchByKeyword(kw, lat, lng, 1, 'accuracy', limit, radius)));
        const seen = new Set();
        const merged = [];
    ...
    rankByMoodRelevance(places, keywords) {
        const BASE = {
        ...
        const scored = places.map((place) => {
            ...
            const label = place.categoryLabel ?? '';
            ...
            const tags = matchedKeyword
                ? [matchedKeyword, ...existingTags.filter((t) => t !== matchedKeyword).slice(0, 2)]
                : existingTags;
    ...
    async saveMoodSearchLog(query, summary, userId) {
    ...
    async getVibeReport(userId, period) {
        if (period === 'monthly') {
            ...
            if (!subscribed) {
                throw new common_1.ForbiddenException('월간 바이브 리포트는 프리미엄 기능이에요. 구독 후 이용해주세요!');
            }
        }
        const now = new Date();
        let startDate;
        ...
        const dailyMoods = period === 'weekly'
            ? Array.from({ length: 7 }, (_, i) => {
                ...
                const dayCheckIns = checkIns.filter((c) => new Date(c.createdAt).toDateString() === dayStr);
                const topMood = dayCheckIns.length > 0
                    ? Object.entries(dayCheckIns.reduce((acc, c) => {
        ...
        const hourBuckets = { 아침: 0, 오후: 0, 저녁: 0, 심야: 0 };
        checkIns.forEach((c) => {
        ...
        const placeCounts = {};
        checkIns.forEach((c) => {
            if (!placeCounts[c.placeId])
                placeCounts[c.placeId] = { name: c.place.name, count: 0 };
