...
let MoodController = class MoodController {
    moodService;
    creditService;
    constructor(moodService, creditService) {
        this.moodService = moodService;
        this.creditService = creditService;
    }
    async search(req, body) {
        const userId = req.user.id;
        const result = await this.moodService.search(body.query, userId, body.lat, body.lng, body.limit, body.radius);
        const txType = result.wasAiSearch ? client_1.CreditTxType.MOOD_SEARCH_AI : client_1.CreditTxType.MOOD_SEARCH_BASIC;
        const cost = result.wasAiSearch ? credit_service_1.CREDIT_COSTS.MOOD_SEARCH_AI : credit_service_1.CREDIT_COSTS.MOOD_SEARCH_BASIC;
