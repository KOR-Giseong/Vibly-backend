...
const receipt_matcher_service_1 = require("../ocr/receipt-matcher.service");
...
const RECEIPT_REQUIRED_CATEGORIES = ['CAFE', 'RESTAURANT', 'BAR'];
const GPS_ONLY_CATEGORIES = ['PARK', 'CULTURAL'];
let PlaceService = PlaceService_1 = class PlaceService {
    ...
    constructor(prisma, kakao, googlePlaces, config, ocr, receiptMatcher, creditService, notificationService) {
        this.prisma = prisma;
        this.kakao = kakao;
        this.googlePlaces = googlePlaces;
        this.config = config;
        this.ocr = ocr;
        this.receiptMatcher = receiptMatcher;
        this.creditService = creditService;
        this.notificationService = notificationService;
    }
    async getNearby(lat, lng, radiusM = 2000, page = 1, limit = 20) {
    ...
    async search(query, lat, lng, page = 1, limit = 20) {
    ...
    async getById(id, userId, hint, mood, vibes) {
        ...
        const score = place.vibeScore > 0
            ...
            : place.rating > 0
                ...
                : 72;
        const categoryVibes = this.generateVibeTags(place.category);
        const vibeTags = mood
    ...
    async toggleBookmark(userId, placeId, imageUrl) {
    ...
    refreshGooglePhotoUrl(url) {
        try {
            const apiKey = this.config.get('GOOGLE_PLACES_API_KEY');
            if (!apiKey || !url.includes('places.googleapis.com'))
                return url;
    ...
    async getBookmarks(userId) {
        ...
        const enriched = await Promise.all(bookmarks.map(async (b) => {
            ...
            const imageUrl = this.refreshGooglePhotoUrl(rawImageUrl ?? CATEGORY_IMAGE[p.category] ?? CATEGORY_IMAGE['ETC']);
    ...
    async getReviews(placeId, page = 1, limit = 20, userId) {
    ...
    async likeReview(userId, reviewId) {
        ...
        if (review.userId !== userId) {
            void this.notificationService.send(review.userId, 'LIKE', '리뷰 좋아요', '누군가 회원님의 리뷰를 좋아해요! ♥', { reviewId }).catch(() => { });
        }
    ...
    async checkInWithReceipt(userId, placeId, receiptBuffer, mood, note, lat, lng) {
        if (placeId.startsWith('kakao_')) {
            await this.upsertKakaoPlace(placeId);
        }
        const place = await this.prisma.place.findUnique({ where: { id: placeId } });
        if (!place)
            throw new common_1.NotFoundException('장소를 찾을 수 없습니다.');
        ...
        if (RECEIPT_REQUIRED_CATEGORIES.includes(category) && !hasReceipt) {
            throw new common_1.BadRequestException(`${this.toCategoryLabel(category)}는 영수증으로만 체크인할 수 있어요.`);
        }
        if (GPS_ONLY_CATEGORIES.includes(category) && !hasGps) {
            throw new common_1.BadRequestException(`${this.toCategoryLabel(category)}는 GPS로만 체크인할 수 있어요.`);
        }
        ...
        if (todayTotal >= 10) {
            throw new common_1.BadRequestException('오늘은 더 이상 체크인할 수 없어요. 하루 최대 10회까지 가능해요.');
        }
        const alreadyToday = await this.prisma.checkIn.count({
            where: { userId, placeId, createdAt: { gte: todayStart, lte: todayEnd } },
        });
        if (alreadyToday > 0) {
            throw new common_1.BadRequestException(`${place.name}은(는) 오늘 이미 체크인했어요. 내일 다시 방문해주세요.`);
        }
        let receiptVerified = false;
        let receiptHash = null;
        if (receiptBuffer && receiptBuffer.length > 0) {
            receiptHash = (0, crypto_1.createHash)('sha256').update(receiptBuffer).digest('hex');
            const duplicateReceipt = await this.prisma.checkIn.findFirst({
                where: { receiptHash },
            });
            if (duplicateReceipt) {
                throw new common_1.BadRequestException('이미 사용된 영수증이에요. 같은 영수증으로 중복 체크인은 불가해요.');
            }
            let lines;
            try {
                ...
                throw new common_1.UnprocessableEntityException('OCR 처리 중 오류가 발생했어요. 잠시 후 다시 시도해주세요.');
            }
            if (!lines.length) {
                throw new common_1.UnprocessableEntityException('영수증에서 텍스트를 인식할 수 없어요. 선명하게 다시 찍어주세요.');
            }
            const { matched, confidence, extractedName } = this.receiptMatcher.matchStoreName(lines, place.name);
            this.logger.log(`[영수증검증] place=${place.name} | extracted=${extractedName} | confidence=${confidence} | matched=${matched}`);
            if (!matched) {
                throw new common_1.UnprocessableEntityException(`영수증 매장명이 일치하지 않아요. 인식된 매장: "${extractedName ?? '없음'}"`);
            }
            receiptVerified = true;
        }
        else {
            if (lat == null || lng == null) {
                throw new common_1.BadRequestException('영수증 또는 현재 위치 정보가 필요해요.');
            }
            if (!place.lat || !place.lng) {
                throw new common_1.UnprocessableEntityException('이 장소는 좌표 정보가 없어 GPS 체크인이 불가해요. 영수증으로 체크인해주세요.');
            }
            const distanceM = this.haversineDistance(lat, lng, place.lat, place.lng);
            this.logger.log(`[GPS검증] place=${place.name} | placeLat=${place.lat} placeLng=${place.lng} | userLat=${lat} userLng=${lng} | distance=${Math.round(distanceM)}m`);
            if (distanceM > 100) {
                throw new common_1.UnprocessableEntityException(`현재 위치가 ${place.name}에서 너무 멀어요. (${Math.round(distanceM)}m 떨어짐, 100m 이내 필요)`);
            }
            ...
            if (recentGpsCheckIn) {
                ...
                throw new common_1.BadRequestException(`GPS 체크인은 같은 장소에서 2시간에 1번만 가능해요. ${remainMin}분 후 다시 시도해주세요.`);
            }
            receiptVerified = false;
        }
        ...
        const rewardType = receiptVerified ? credit_service_1.CreditTxType.CHECKIN_RECEIPT : credit_service_1.CreditTxType.CHECKIN_GPS;
        const rewardAmount = receiptVerified ? credit_service_1.CREDIT_REWARDS.CHECKIN_RECEIPT : credit_service_1.CREDIT_REWARDS.CHECKIN_GPS;
        this.creditService.earn(userId, rewardAmount, rewardType, checkIn.id).catch((err) => this.logger.error(`체크인 크레딧 지급 실패 userId=${userId}`, err));
    ...
    async upsertKakaoPlace(kakaoId, place, hint) {
        try {
    ...
    async fetchKakaoPlaceById(kakaoPlaceId, hint) {
    ...
    personalizeByMood(mood, categoryVibes) {
        const moodMap = {
    ...
    personalizeByVibes(preferredVibes, categoryVibes) {
    ...
    async smartRecommend(userId, lat, lng, mode = 'nearby') {
        const subscribed = await this.creditService.isSubscribed(userId);
        if (!subscribed) {
            throw new common_1.ForbiddenException('실시간 추천은 프리미엄 기능이에요. 구독 후 이용해주세요!');
        }
        const WMO_CODES = {
        ...
        let weather = '맑음';
        try {
            const weatherRes = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current_weather=true`, { signal: AbortSignal.timeout(5000) });
            if (weatherRes.ok) {
        ...
        let timeOfDay;
        if (kstHour >= 5 && kstHour < 12)
            timeOfDay = '아침';
        else if (kstHour >= 12 && kstHour < 17)
            timeOfDay = '오후';
        else if (kstHour >= 17 && kstHour < 22)
            timeOfDay = '저녁';
        else
            timeOfDay = '심야';
...
형식: {"message": "한 줄 추천 멘트 (20자 이내)", "keywords": ["키워드1", "키워드2", "키워드3"]}`;
        ...
        const results = await Promise.all(mode === 'wide'
    ...
    async getReviewSummary(placeId, userId) {
        const subscribed = await this.creditService.isSubscribed(userId);
        if (!subscribed) {
            throw new common_1.ForbiddenException('AI 리뷰 요약은 프리미엄 기능이에요. 구독 후 이용해주세요!');
        }
...
exports.PlaceService = PlaceService = PlaceService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        ...
        receipt_matcher_service_1.ReceiptMatcherService,
