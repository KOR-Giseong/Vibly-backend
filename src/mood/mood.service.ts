...
export class MoodService {
  ...
  private static readonly LOCATION_NAMES: string[] = [
    ...
    // ── 서울 동네/상권 단위 ───────────────────────────────────────────────────
    '합정','홍대','이태원','신촌','성수','서울숲','건대','혜화','명동','잠실','판교','여의도','압구정',
    '청담','삼성','역삼','선릉','논현','신사','가로수길','인사동','북촌','삼청동','연남','상수','망원',
    '공덕','왕십리','뚝섬','한남','반포','경리단길','해방촌','상암','상계','창동','방학','쌍문',
    '을지로','익선동','낙원동','충무로','동대문',
    ...
    // ── 대구 / 울산 / 부산 ────────────────────────────────────────────────────
    '대구','동성로','수성못','수성구','달서구','달성군','울산','남구','북구',
  ...
  private tryQuickMatch(query: string, regionLabel?: string): GeminiAnalysis | null {
    ...
    // 카카오 정확도를 위해 더 구체적인 검색어로 리매핑
    const LOCATION_REMAP: Record<string, string> = {
      '을지로': '을지로3가',   // 1가는 종각 옆이라 섞임
      '동성로': '대구 동성로',  // 단독 검색 시 서울 결과 나올 수 있음
      '충장로': '광주 충장로',
      '둔산':   '대전 둔산동',
    };
